  ...
  return (
    <div className="flex items-center justify-around md:w-36 lg:w-52 2xl:w-80">
      ...
      }
      {
        isPlaying
          ?
          <BsFillPauseFill size={45} color="#FFF" onClick={()=>handlePlayPause(false)} className="cursor-pointer" />
          :
          <BsFillPlayFill size={45} color="#FFF" onClick={()=>handlePlayPause(true)} className="cursor-pointer" />
      }
      {
        currentSongs?.length &&
      ...
      {/* shuffle button */}
      <BsShuffle size={20} color={shuffle ? 'red' : 'white'} onClick={() => setShuffle(pre => !pre)} className="hidden sm:block cursor-pointer" />
    </div>
  )
}
